...
const errs = []; const titles = new Map(); const descs = new Map();
let links = 0;
for (const p of pages) {
  ...
  if (!/rel="canonical"/.test(html)) errs.push(`${u}: no canonical`);
  if (!/hreflang="es"/.test(html) || !/hreflang="en"/.test(html)) errs.push(`${u}: hreflang missing`);
  ...
  for (const m of html.matchAll(/hreflang="(en|es)" href="https:\/\/elpasolawyers\.org([^"]+)"/g)) if (!exists(m[2])) errs.push(`${u}: hreflang target missing ${m[2]}`);
}
...
console.log(`${pages.length} pages, ${links} links checked, ${smUrls.length} sitemap urls`);
for (const p of pages) { const u = urlOf(p); if (u === '/404' || u === '/404.html') continue; if (!smUrls.includes('https://elpasolawyers.org' + u)) errs.push(`sitemap missing ${u}`); }
